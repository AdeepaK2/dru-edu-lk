...
export default function StudentSheetsPage() {
  ...
  if (authLoading || loading) {
    return (
      <div className={`min-h-screen bg-gradient-to-br ${theme === 'ben10' ? 'from-[#64cc4f] to-[#222222]' : theme === 'tinkerbell' ? 'from-green-400 via-green-500 to-yellow-600' : theme === 'bounceworld' ? 'bg-gradient-to-br from-white via-[#1D428A] to-[#C8102E]' : 'from-blue-400 to-indigo-600'} flex items-center justify-center`}>
  ...
  return (
    <div key={`sheets-${theme}`} className={`min-h-screen bg-gradient-to-br ${theme === 'ben10' ? 'from-[#64cc4f] via-[#b2e05b] to-[#222222]' : theme === 'tinkerbell' ? 'from-green-400 via-green-500 to-yellow-600' : theme === 'bounceworld' ? 'bg-gradient-to-br from-white via-[#1D428A]/20 to-[#C8102E]/20' : 'from-blue-600 via-indigo-400 to-blue-500'} p-6`}>
      <div className="max-w-4xl mx-auto">
        {/* Theme-aware Header */}
        <div className={`bg-gradient-to-r ${theme === 'ben10' ? 'from-[#64cc4f] to-[#222222]' : theme === 'tinkerbell' ? 'from-green-400 via-green-500 to-yellow-600' : theme === 'bounceworld' ? 'from-[#1D428A] via-white to-[#C8102E]' : 'from-blue-400 to-blue-500'} rounded-3xl shadow-2xl border-4 ${theme === 'bounceworld' ? 'border-[#1D428A]' : 'border-black'} p-8 mb-8 relative overflow-hidden`}>
          ...
          <div className="flex items-center space-x-4 relative z-10">
            ...
            <div>
              ...
              <p className={`font-bold text-lg ${theme === 'ben10' ? 'text-white' : theme === 'tinkerbell' ? 'text-white' : theme === 'bounceworld' ? 'text-white' : 'text-blue-200'}`}>
                {theme === 'bounceworld'
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <div className={`bg-white rounded-lg shadow-lg p-6 border-4 ${theme === 'bounceworld' ? 'border-[#1D428A]' : 'border-black'} relative overflow-hidden`}>
            ...
            <div className="flex items-center justify-between">
              ...
              <div className={`text-6xl ${theme === 'bounceworld' ? 'text-[#C8102E]' : 'text-black'}`}>📊</div>
            </div>
          </div>
        ...
        <div className={`bg-white rounded-3xl shadow-2xl border-4 ${theme === 'bounceworld' ? 'border-[#1D428A]' : 'border-black'} overflow-hidden`}>
          <div className={`bg-gradient-to-r ${theme === 'ben10' ? 'from-[#48a735]  to-[#222222]' : theme === 'tinkerbell' ? 'from-green-400 via-green-500 to-yellow-600' : theme === 'bounceworld' ? 'from-[#1D428A] via-white to-[#C8102E]' : 'from-blue-400 via-blue-500 to-blue-600'} p-6 border-b-4 ${theme === 'bounceworld' ? 'border-[#1D428A]' : 'border-black'}`}>
            ...
            <p className={`font-bold ${theme === 'ben10' ? 'text-[#b2e05b]' : theme === 'tinkerbell' ? 'text-yellow-200' : theme === 'bounceworld' ? 'text-[#1D428A]' : 'text-blue-200'}`}>
              {theme === 'bounceworld' ? 'Click on a class to slam dunk your Google Sheets assignments! 🏀' : 'Click on a class to view and access your sheets'}
            </p>
          </div>

          {classes.length === 0 ? (
            <div className={`bg-gradient-to-r ${theme === 'ben10' ? 'from-[#222222] via-[#64cc4f] to-[#b2e05b]' : theme === 'tinkerbell' ? 'from-green-400 via-green-500 to-yellow-600' : theme === 'bounceworld' ? 'from-white via-[#1D428A]/20 to-[#C8102E]/20' : 'from-blue-400 via-indigo-500 to-blue-600'} p-12 text-center border-t-4 ${theme === 'bounceworld' ? 'border-[#1D428A]' : 'border-black'}`}>
              <div className="text-6xl mb-6">{theme === 'ben10' ? '🦸‍♂️' : theme === 'tinkerbell' ? '🧚‍♀️' : theme === 'bounceworld' ? '🏀' : ''}</div>
              <h3 className="text-2xl font-black text-black mb-4">
                {theme === 'bounceworld' ? 'No Classes Yet - Time to Start Your Game!' : 'No Classes Yet'}
              </h3>
              <p className={`font-bold text-lg ${theme === 'ben10' ? 'text-[#b2e05b]' : theme === 'tinkerbell' ? 'text-yellow-200' : theme === 'bounceworld' ? 'text-[#1D428A]' : 'text-blue-200'}`}>
                {theme === 'bounceworld' ? 'You haven\'t enrolled in any classes yet. Time to start your training and slam dunk those assignments! 🏀' : 'You haven\'t enrolled in any classes yet. Time to start your training!'}
              </p>
            </div>
          ) : (
            <div className="divide-y-4 divide-black">
              {classes.map((classData) => (
                <div
                  key={classData.id}
                  className={`bg-gradient-to-r ${theme === 'ben10' ? 'from-[#64cc4f]  to-[#222222]' : theme === 'tinkerbell' ? 'from-green-400 via-green-500 to-yellow-600': theme === 'bounceworld' ? 'from-white via-[#1D428A]/10 to-[#C8102E]/10' : 'from-blue-400 to-blue-600'} p-6 hover:scale-105 transition-all cursor-pointer border-b-4 ${theme === 'bounceworld' ? 'border-[#1D428A]' : 'border-black'} last:border-b-0`}
                  onClick={() => openClassSheets(classData)}
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-start space-x-4">
                      <div className="flex-shrink-0">
                        <div className={`w-16 h-16 bg-gradient-to-br ${theme === 'ben10' ? 'from-[#64cc4f] to-[#b2e05b]' : theme === 'tinkerbell' ? 'from-yellow-400 to-green-500' : theme === 'bounceworld' ? 'from-[#1D428A] to-[#C8102E]' : 'from-blue-400 to-indigo-600'} rounded-2xl flex items-center justify-center border-4 ${theme === 'bounceworld' ? 'border-[#1D428A]' : 'border-black'} shadow-lg`}>
                      ...
                      <div className="flex-1">
                        ...
                        <p className={`font-bold text-lg mb-1 ${theme === 'ben10' ? 'text-[#b2e05b]' : theme === 'tinkerbell' ? 'text-white' : theme === 'bounceworld' ? 'text-[#1D428A]' : 'text-blue-200'}`}>{classData.subject}</p>
                        <p className={`font-bold text-sm mb-3 ${theme === 'ben10' ? 'text-[#123a0a]' : theme === 'tinkerbell' ? 'text-white' : theme === 'bounceworld' ? 'text-[#C8102E]' : 'text-blue-300'}`}>{classData.year}</p>

                        <div className={`flex items-center space-x-6 text-sm font-bold ${theme === 'ben10' ? 'text-black' : theme === 'tinkerbell' ? 'text-black' : theme === 'bounceworld' ? 'text-[#1D428A]' : 'text-blue-200'}`}>
                    ...
                    <div className="flex items-center space-x-3">
                      {classData.sheetCount > 0 && (
                        <span className={`inline-flex items-center px-4 py-2 rounded-full text-sm font-black border-2 ${theme === 'bounceworld' ? 'border-[#1D428A]' : 'border-black'} shadow-lg ${theme === 'ben10' ? 'bg-[#64cc4f] text-white' : theme === 'tinkerbell' ? 'bg-yellow-500 text-white' : theme === 'bounceworld' ? 'bg-[#C8102E] text-white' : 'bg-blue-500 text-white'}`}>
                          {classData.sheetCount} Sheet{classData.sheetCount !== 1 ? 's' : ''}
                        </span>
                      )}
                      <div className={`text-3xl transform group-hover:translate-x-1 transition-transform ${theme === 'bounceworld' ? 'text-[#1D428A]' : 'text-white'}`}>➡️</div>
                    </div>
                  </div>
                </div>
              ))}
