...
export default function StudentResults() {
        ...
        const classProgress = await Promise.all(
          activeEnrollments.map(async (enrollment) => {
            try {
              ...
              // Get test submissions for this student in this class
              const submissions = await SubmissionService.getSubmissionsByStudentAndClass(
                student.id, 
                enrollment.classId
              );
              ...
              const scores = submissions.map((s: any) => {
                ...
                const totalScore = s.totalScore || s.autoGradedScore || 0;
              ...
              if (scores.length >= 4) {
                ...
                const firstAvg = firstHalf.reduce((sum: number, s: number) => sum + s, 0) / firstHalf.length;
                const secondAvg = secondHalf.reduce((sum: number, s: number) => sum + s, 0) / secondHalf.length;
                improvement = secondAvg - firstAvg;
              }
              
              // Get last test date
              const lastTestDate = submissions.length > 0 
                ? new Date(Math.max(...submissions.map((s: any) => s.submittedAt?.toDate().getTime() || 0)))
                : undefined;
              ...
              return {
                ...
                totalTests: 0, // Will be updated when we get all tests for the class
                completedTests,
                averageScore: Math.round(averageScore),
                improvement: Math.round(improvement),
                lastTestDate,
                difficultTopics: [], // Will be calculated from detailed analysis
                strongTopics: [], // Will be calculated from detailed analysis
  ...
  const loadClassResults = async (classId: string) => {
    ...
    try {
      // Get all test submissions for this student in this class
      const submissions = await SubmissionService.getSubmissionsByStudentAndClass(
        student.id, 
        classId
      );
      ...
      const detailedResults = await Promise.all(
        submissions.map(async (submission: any) => {
          try {
            ...
            const totalScore = submission.totalScore || submission.autoGradedScore || 0;
            ...
            return {
              ...
              subject: test?.subjectName || 'Unknown',
              className: studentClasses.find(c => c.id === classId)?.name || 'Unknown Class',
              score: Math.round(scorePercentage),
              totalQuestions: submission.finalAnswers?.length || 0,
              correctAnswers: submission.mcqResults?.filter((r: any) => r.isCorrect).length || 0,
              completedAt: submission.submittedAt?.toDate() || new Date(),
              timeSpent: submission.timeSpent || 0,
              lessonTopics: [], // Will be populated from question topics
              difficultQuestions: submission.mcqResults?.filter((r: any) => !r.isCorrect) || []
            } as TestResult;
          } catch (err) {
            console.error(`Error loading test details for submission ${submission.id}:`, err);
            return null;
          }
        })
      );
      
      const validResults = detailedResults.filter((r: any) => r !== null) as TestResult[];
      setTestResults(validResults);
      
      // Analyze difficult topics
      analyzeDifficultTopics(validResults);
                      ...
                      <div className="space-y-4">
                        {testResults.map((result) => (
                          ...
                          >
                            ...
                            <div className="mt-3 flex items-center justify-between text-xs text-gray-500 dark:text-gray-400">
                              <span>
                                Time: {Math.round(result.timeSpent / 60)} min
                              </span>
                              <span>
